...
- **自动化知识图谱构建**：从文本中抽取实体、关系与属性，生成结构化三元组，作为图谱检索载体。  
- **知识图谱迭代优化**：通过校验、补充与去重机制，持续提升图谱覆盖与质量。  
- **双源 RAG 检索与融合**：同时检索知识图谱与文档库，将多源证据注入生成模型，返回带证据的回答并支持来源溯源。  
- **交互式可视化**：知识图谱与证据链的可视化展示与交互，便于人工验证与探索。
