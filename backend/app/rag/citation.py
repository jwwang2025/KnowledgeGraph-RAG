...
class CitationSet:
    ...
    def get_sorted_by_relevance(self) -> List[Citation]:
        """按相关性排序"""
        return sorted(self.citations, key=lambda x: x.relevance_score, reverse=True)
    ...
    def format_for_display(self) -> str:
        ...
        for i, c in enumerate(self.get_sorted_by_relevance(), 1):
            ...
            lines.append(f"    {c.excerpt[:100]}{'...' if len(c.excerpt) > 100 else ''}")
...
class CitationGenerator:
    ...
    def generate_triple_citations(triples: List[tuple], 
        ...
        Args:
            triples: 三元组列表 [(实体1, 关系, 实体2), ...]
            query: 原始查询
            relevance_scores: 各三元组的相关性得分（可选）
        ...
        for i, triple in enumerate(triples):
            ...
            # 生成引用片段
            excerpt = f"({triple[0]} {triple[1]} {triple[2]})"
            ...
            citation = Citation(
                ...
                source_id=source_id,
                source_name=f"知识图谱:{triple[0]}",
                original_text=excerpt,
                excerpt=excerpt,
                position=(0, len(excerpt)),
                related_entities=[triple[0], triple[2]],
    ...
    def generate_wiki_citation(wiki_result: Dict[str, str],
        ...
        Args:
            wiki_result: Wikipedia 检索结果 {"title": ..., "summary": ...}
            query: 原始查询
...
class CitationEmbedder:
    ...
    def __init__(self, format_type: str = "superscript"):
        ...
        Args:
            format_type: 嵌入格式 ("superscript", "footnote", "inline")
        """
    ...
    def embed_citations(self, text: str, 
        ...
        Args:
            text: 原始文本
            citations: 要嵌入的引用列表
            matched_segments: 匹配的文本片段及其对应引用索引 [(片段, citation_index), ...]
        ...
        citation_map = {}  # source_id -> new_index
        
        for idx, citation in enumerate(sorted_citations):
            citation_map[citation.source_id] = idx + 1
        ...
        for segment, citation_idx in matched_segments:
            if citation_idx < len(sorted_citations):
                ...
                if ref_num:
                    if self.format_type == "superscript":
                        replacement = f"{segment}[{ref_num}]"
                    elif self.format_type == "inline":
        ...
        # 返回使用了的引用
        used_citations = [sorted_citations[i] for i in sorted(used_indices)]
        
        return result_text, used_citations
